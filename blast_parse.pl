...
if ($#ARGV == -1) {
	...
	print " -num_align X 	only print X alignments.\n";	
	print "\n";
	exit;
}
...
foreach my $argnum (0 .. $#ARGV) {
	...
	if ($ARGV[$argnum] =~ "-num_align") {
		$num_align = $ARGV[$argnum+1];
	}	
}
...
while (my $seq = $seq_input->next_seq) {
	...
	my $factory = Bio::Tools::Run::StandAloneBlastPlus->new(-db_name => $db);
	my $rep;
	if ($prog eq "blastp") {
		$rep = $factory->blastp(-query => $seq_source, -outfile => $outfile, -method_args => [-evalue => 1E-10, -num_alignments => $num_align]);
	}
	elsif ($prog eq "blastn") {
		$rep = $factory->blastn(-query => $seq_source, -outfile => $outfile, -method_args => [-evalue => 1E-10, -num_alignments => $num_align]);
	}	
	elsif ($prog eq "blastx") {
		$rep = $factory->blastx(-query => $seq_source, -outfile => $outfile, -method_args => [-evalue => 1E-10, -num_alignments => $num_align]);
	}	
	elsif ($prog eq "tblastn") {
		$rep = $factory->tblastn(-query => $seq_source, -outfile => $outfile, -method_args => [-evalue => 1E-10, -num_alignments => $num_align]);
	}		
	$factory->cleanup;	
